...
/*=======================================================find users by Role======================================================*/
	...
	public ResponseEntity<ResponseStructure<List<UserResponse>>> findAllByRole(int programId, String userRole) {
		...
		return academicProgramRepository.findById(programId)
				.map(academicProgram -> {          //value of (userRole in uppercase) is defined by us
					...
					if(EnumSet.allOf(UserRole.class).contains(roleOfUser)){
						...
						//map to userReponse does not accept list,so we use strem()
						List<UserResponse> collect = listOfUsers.stream()
						.map(this::mapToUserResponse)
						//collecting list of users
...
/*note:
 *   soft delete:
 *           data is deleted in UI but not in database*/
